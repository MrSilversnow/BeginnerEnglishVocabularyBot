def select_column_names(engine, table_name):
    ...
    result = engine.execute(f"""select distinct column_name from INFORMATION_SCHEMA.COLUMNS where table_name='{table_name}'
                            and table_schema='bot' order by ordinal_position""").fetchall()
...
def select_topic(engine, tid=None, name=None):
    ...
    if tid is None and name is None:
        result = engine.execute("select * from topics")
    elif name is None:
